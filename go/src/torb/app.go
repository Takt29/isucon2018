...
func getEventMini(eventID, loginUserID int64) (*Event, error) {
	var event Event
	...
	rows, err := db.Query("SELECT rank, MAX(price) FROM sheets GROUP BY rank")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		db.QueryRow("SELECT COUNT(*) FROM reservations as r, sheets as s WHERE r.event_id = ? AND r.sheet_id = s.id AND s.rank = ? AND r.canceled_at IS NULL", eventID, sheet.Rank).Scan(&reserved)
		event.Sheets[sheet.Rank].Total = total
...
func main() {
	...
	e.GET("/api/users/:id", func(c echo.Context) error {
		...
		rows, err = db.Query("SELECT event_id FROM reservations WHERE user_id = ? GROUP BY event_id ORDER BY MAX(changed_at) DESC LIMIT 5", user.ID)
		if err != nil {
			return err
		}
		defer rows.Close()
